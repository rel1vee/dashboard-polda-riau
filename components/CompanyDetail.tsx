                  ...
                  <div className="h-[400px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <ComposedChart data={periodData}>
                        ...
                        <XAxis
                          dataKey="periode"
                          domain={["dataMin", "dataMax"]} // Pastikan sumbu X mencakup semua data
                          padding={{ left: 0, right: 0 }} // Tambahkan padding agar garis tidak terpotong
                        />
                        <YAxis
                          domain={[0, "dataMax"]} // Pastikan sumbu Y dimulai dari 0 hingga nilai maksimal data
                        />
                        <Tooltip
                          content={({ active, payload }) => {
                            if (active && payload && payload.length) {
                              return (
                                <div className="p-4 bg-white border rounded shadow">
                                  ...
                                  <p className="text-[#16a34a]">
                                    Target Monokultur :{" "}
                                    {payload[0]?.payload?.monoTarget ?? 0} Ha
                                  </p>
                                  <p className="text-[#22c55e]">
                                    Capaian Monokultur :{" "}
                                    {payload[0]?.payload?.monoAchievement ?? 0}{" "}
                                    Ha
                                  </p>
                                  <p className="text-[#00008B]">
                                    Target Tumpang Sari :{" "}
                                    {payload[0]?.payload?.tsTarget ?? 0} Ha
                                  </p>
                                  <p className="text-[#0000FF]">
                                    Capaian Tumpang Sari :{" "}
                                    {payload[0]?.payload?.tsAchievement ?? 0} Ha
                                  </p>
                                </div>
                              );
                            }
