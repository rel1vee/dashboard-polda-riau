...
const DashboardPoldaRiauPage = () => {
  ...
  const getTotalTargets = () => {
    return programDua.reduce(
      (acc, polres) => {
        const polsekTarget = polres.polsek7.reduce(
          (total, polsek) =>
            total +
  ...
  const getTotalAchievements = () => {
    return programDua.reduce(
      (acc, polres) => {
        ...
        const polsekAchievement = polres.polsek7.reduce(
          (total, polsek) =>
            total +
            (polsek.villages.reduce(
              (achievement, village) =>
                ...
                >
                  <Card className="border-none rounded-xl shadow-lg hover:shadow-xl transition-all">
                    <CardHeader className="rounded-t-xl bg-gradient-to-r from-blue-50 to-indigo-50">
                      ...
                      {selectedCity ? (
                        <CardDescription>
                          Total {selectedCity.polsek7.length} POLSEK. Pilih
                    ...
                    <CardContent className="p-0 overflow-y-auto max-h-[500px]">
                      {selectedCity ? (
                        <div className="p-4">
                          {selectedCity.polsek7.length > 0 ? (
                            <Table>
                              <TableBody>
                                {selectedCity.polsek7
                                  .map((polsek) => ({
                                    ...polsek,
